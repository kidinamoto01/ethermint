...
func (app *EthermintApplication) CheckTx(txBytes []byte) abciTypes.ResponseCheckTx {
	...
	if err != nil {
		...
		return abciTypes.ResponseCheckTx{Code: code.CodeTypeBadNonce}
	}
...
func (app *EthermintApplication) DeliverTx(txBytes []byte) abciTypes.ResponseDeliverTx {
	...
	if err != nil {
		...
		return abciTypes.ResponseDeliverTx{Code: code.CodeTypeBadNonce}
	}
...
func (app *EthermintApplication) BeginBlock(req abciTypes.RequestBeginBlock) abciTypes.ResponseBeginBlock{
	...
	app.backend.UpdateHeaderWithTimeInfo(req.Header)
...
func (app *EthermintApplication) Commit() abciTypes.ResponseCommit {
	...
	if err != nil {
		...
		return abciTypes.ResponseCommit{Code: code.CodeTypeBadOption}
	}
	state, err := app.getCurrentState()
	if err != nil {
		app.logger.Error("Error getting latest state", "err", err) // nolint: errcheck
		return abciTypes.ResponseCommit{Code: code.CodeTypeUnauthorized}
	}

	app.checkTxState = state.Copy()
	return abciTypes.ResponseCommit{Code: code.CodeTypeOK,Data:blockHash.Bytes()}
}
...
func (app *EthermintApplication) Query(query abciTypes.RequestQuery) abciTypes.ResponseQuery {
	...
	if err := json.Unmarshal(query.Data, &in); err != nil {
		return abciTypes.ResponseQuery{Code: code.CodeTypeBadOption, Log: err.Error()}
	}
	...
	return abciTypes.ResponseQuery{Code: code.CodeTypeOK, Value: bytes}
}
...
func (app *EthermintApplication) validateTx(tx *ethTypes.Transaction) abciTypes.ResponseCheckTx {
	...
	if tx.Size() > maxTransactionSize {
		return abciTypes.ResponseCheckTx{Code: code.CodeTypeBadOption}
	}
